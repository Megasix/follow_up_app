...
class DatabaseService {
  ...
  static Future<void> addInactiveInstructor(String schoolId, UserData data) {
    return schoolsCollection
        .doc(schoolId)
        .collection('school.inactiveInstructors')
        .doc(data.uid)
        .set(data.toMap());
  }

  static Future<void> addChatroom(
      String userId, ChatroomData chatroomData) async {
  ...
  static Future<List<RideData>> getRides(String userId) {
    return usersCollection
        ...
        .get()
        .then<List<RideData>>(_ridesFromSnapshotMap);
  }
  ...
  static Stream<List<ChatroomData>> streamStudentChatrooms(UserData userData) {
    return chatRoomCollection
        .where('members',
            arrayContains: ChatUserData.fromUserData(userData).toMap())
        .snapshots()
        .map<List<ChatroomData>>(_chatroomsFromSnapshot);
  }

//get all chatrooms the signed in school has
  static Stream<List<ChatroomData>> streamSchoolChatrooms(
      SchoolData schoolData) {
    return chatRoomCollection
        .where('members',
            arrayContains: ChatUserData.fromSchoolData(schoolData).toMap())
        .snapshots()
        .map<List<ChatroomData>>(_chatroomsFromSnapshot);
  }
  ...
  static List<UserData> _usersFromSnapshot(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) => queryDocSnap.data() as UserData)
        .toList();
  }

  static List<ChatroomData> _chatroomsFromSnapshot(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) => queryDocSnap.data() as ChatroomData)
        .toList();
  }

  static List<RideData> _ridesFromSnapshot(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) => queryDocSnap.data() as RideData)
        .toList();
  }

  static List<ChatUserData> _chatUsersFromUsersSnapshotMap(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) =>
            ChatUserData.fromUserData(queryDocSnap.data() as UserData))
        .toList();
  }

  static List<UserData> _usersFromSnapshotMap(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) => UserData.fromMap(
            queryDocSnap.id, queryDocSnap.data() as Map<String, dynamic>))
        .toList();
  }

  static List<ChatMessage> _chatMessagesFromSnapshotMap(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) =>
            ChatMessage.fromMap(queryDocSnap.data() as Map<String, dynamic>))
        .toList();
  }

  static List<RideData> _ridesFromSnapshotMap(
      QuerySnapshot<Object?> querySnapshot) {
    return querySnapshot.docs
        .map((queryDocSnap) => RideData.fromMap(
            queryDocSnap.id, queryDocSnap.data() as Map<String, dynamic>))
        .toList();
  }
}
